...
{
    public class StratusRedStar : StratusStar<StratusWhipNPCDebuffRed>, IDrawPixelated
    {
        VertexStrip _strip;

        public override float RotateSpeed => 1;
        public override float Radius => 400;
        ...
        public override void PostAI()
        {
            ...
            {
                Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, Main.rand.NextBool() ? DustID.Enchanted_Pink : DustID.RedTorch, Main.rand.NextFloat(-0.25f, 0.25f), Main.rand.NextFloat(-0.25f, 0.25f), 150, Color.White, 1.3f);
            }
            if (Main.rand.NextBool(6))
            {
                ...
                {
                    Gore.NewGore(Projectile.position, Projectile.velocity * 0.2f, ModContent.GoreType<StratusRedStarGore>());
                }
            }
        }
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 12; i++)
            {
                ...
                Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, Main.rand.NextBool() ? DustID.Enchanted_Pink : DustID.RedTorch, speed.X, speed.Y, 150, Color.White, 1.3f);
            }

            for (int i = 0; i < 16; i++)
            {
                Vector2 speed = Utils.RandomVector2(Main.rand, 4, 5).RotatedByRandom(Math.PI * 2);
                Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, Main.rand.NextBool() ? DustID.Enchanted_Pink : DustID.RedTorch, speed.X, speed.Y, 150, Color.White, 1.1f);
            }

            for (int i = 0; i < 8; i++)
            {
                Vector2 speed = Projectile.rotation.ToRotationVector2().RotatedByRandom(0.3f) * 0.5f;
                Gore.NewGore(Projectile.position, speed, ModContent.GoreType<StratusRedStarGore>());
            }
        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Main.spriteBatch.Draw(
                ...
                position: Projectile.Center - Main.screenPosition,
                sourceRectangle: texture.Frame(),
                color: new Color(1, 0.5f, 0.5f, 0.5f),
            ...
            Main.spriteBatch.Draw(
                ...
                position: Projectile.Center - Main.screenPosition,
                sourceRectangle: texture.Frame(),
                color: new Color(1, 0.2f, 0.2f, 0.8f),
        ...
        public void DrawPixelated()
        {
            ...
            effect.Parameters["uTexturePalette0"].SetValue(AssetDirectory.Textures.Extra.Palettes.RedStarHueTrail.Value);
